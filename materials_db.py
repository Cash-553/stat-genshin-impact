...
# ⚠⚠ 目前停在 0（= 不重置） ⚠⚠
#
# 2026-09-21 用官方名单 `generated_names.py`（材料 574 + 圣遗物 299）核对过：
#
#   开发版材料库 147 个（内置 71 + 自动 76）
#       自动登记里 73 个在白名单里（**是真名字**），只有 3 个是 OCR 错名：
#           牢固的箭镞 / 锐利的箭镞 / 历战的箭镞   ← 白名单里是「箭簇」
#       内置里有 7 个不在白名单（待人工确认）：
#           沉重的骨片 / 浮游核 / 浮游晶核 / 蚀灭的鳞羽 / 蚀灭的灵犀 /
#           蚀灭的阳焰 / 残缺的剑柄
#
#   所以**两边都有对有错，一刀切重置会误删真名字**。
#   结论：不做自动重置，改成在「材料库」编辑器里按白名单**逐个标出疑似错名**
#   （红色 + 「疑似」标签，还有「只看疑似错名」筛选），让用户自己决定删哪个。
#
# 0 -> 1：清掉自动登记的名字，只留内置那份（INITIAL_MATERIALS）
LIB_VERSION = 0
